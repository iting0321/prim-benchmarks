...
int main(int argc, char **argv) {
    ...
    unsigned int i = 0;
    T accum = 0; // data pointer(4 bytes)
        ...
        while (offset < input_size) {
    ...
    #if PRINT
            {
                ...
                DPU_FOREACH(dpu_set, dpu, i) {
                    ...
                    //printf("DPU %d, Accumulated Value: %lu, Intermediate Sum: %lu\n", i, accum, temp);
                }
                ...
                for (unsigned int i = 0; i < nr_of_dpus; i++) {
                    T last_sum = results[i].t_count; // Last sum from the previous chunk
                    results_scan[i] = partial_sum;  // Store the adjustment for DPU `i`
                    partial_sum += last_sum;        // Update the partial sum for the next DPU
                }
                **/
                ...
                for(i = 0; i < nr_of_dpus; i++) {
                    input_arguments_2[i].size = input_size_dpu_round_chunk * sizeof(T);
                    input_arguments_2[i].kernel = kernel;
                    input_arguments_2[i].t_count = results_scan[i];
                }
                //printf("==========%d=========\n",input_size_dpu_round_chunk * sizeof(T));
                DPU_FOREACH(dpu_set, dpu, i) {
